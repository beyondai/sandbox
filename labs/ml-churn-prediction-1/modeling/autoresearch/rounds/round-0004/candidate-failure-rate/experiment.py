"""Candidate: Logistic Regression (C=0.1) plus ticket_rate and failure_rate features.

Hypothesis: round 3 found that rate-normalizing support_tickets_last_90d by
tenure (ticket_rate) helped. payment_failures_last_90d was the other top
churn driver by coefficient magnitude, so the same rate-normalization pattern
is applied to it: failure_rate = payment_failures_last_90d /
(tenure_months + 1).
...
"balanced", max_iter=1000, random_state=0). ticket_rate and failure_rate are
added to the StandardScaler numeric feature list alongside the four existing
numeric features.

Note on DATA_PATH: this file lives 5 directories under the project root
(modeling/autoresearch/rounds/round-0004/candidate-failure-rate/), not 2 like
...
def main():
    ...
    df["failure_rate"] = df["payment_failures_last_90d"] / (df["tenure_months"] + 1)
