"""Candidate: Logistic Regression (C=0.1) plus an engineered usage_per_tenure
feature, added alongside the existing winning ticket_rate feature.

Hypothesis: usage_per_tenure = monthly_usage_hours / (tenure_months + 1) is a
"how actively engaged is this customer relative to how long they've been
around" signal, analogous to ticket_rate but built from usage instead of
support tickets.
...
"balanced", max_iter=1000, random_state=0). usage_per_tenure is added to the
StandardScaler numeric feature list alongside the five existing numeric
features (including ticket_rate).

Note on DATA_PATH: this file lives 5 directories under the project root
(modeling/autoresearch/rounds/round-0005/candidate-usage-rate/), not 2 like
