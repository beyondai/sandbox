...
def analyze_ab(control, treatment, alpha=0.05):
    ...
    return {"control_rate": p_c, "treatment_rate": p_t,
            "lift": (p_t - p_c) / p_c,
...
`../../ml-modeling/scripts/hypothesis_tester.py` runs this (plus Welch's t-test,
paired t-test, chi-square) against a real CSV:

```
python3 hypothesis_tester.py proportion \
  --successes-a 120 --trials-a 1000 --successes-b 145 --trials-b 1000
python3 hypothesis_tester.py ttest --file data.csv \
