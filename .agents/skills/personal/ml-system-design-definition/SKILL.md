...
- **Problem**: core problem, why now, user pain point, link to team/company goals.
- **Requirements — scope**: must-have features; explicit out-of-scope list for this version. Most-skipped item in review — check it first.
- **Requirements — non-functional**: expected load, latency (e.g. p99), availability target.
- **Metrics — offline**: static-dataset metrics (AUC, nDCG, precision/recall).
- **Metrics — online**: live A/B metrics (CTR, conversion, session time) plus guardrail metrics that must not regress.
- **Team**: stakeholders to inform, collaborating teams, blocking/blocked dependencies.
